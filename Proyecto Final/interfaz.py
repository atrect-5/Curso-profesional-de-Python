...
# Importamos los modulos que usaremos incluyendo el que realizamos anteriormente
...
class App:
    ...
    def __init__(self):
        ...
        # Es el metodo que se usara cuando se cierre la ventana
        self.root.protocol("WM_DELETE_WINDOW", self.exit)

        self.root.withdraw() # El metodo 'withdraw' es ejecutado para ocultar una interfaz grafica luego de ser creada
        ...
        # Usamos el modulo de tkcalendar para crear la vista del calendario
        ...
        # Boton para ver registros del dia
        ttk.Button(self.top, text='Ver eventos',
        ...
        ttk.Button(self.top, text='Nuevo Evento',
                   command=lambda: EventoForm(self.root)).grid(sticky="ew", pady=5, padx=5)

        # Loop para no cerrar la ventana
        self.root.mainloop()


    def exit(self):
        '''Cierra la conexión a base de datos y todas las interfaces gráficas'''
...
class ListView:
    ...
    def insert_events(self):
        ...
        for event in self.day_events:
            self.eventos.insert(self.day_events.index(event), str(event.id) + " | " + str(event))

    def event_detail(self):
        if self.eventos.curselection():
            id = self.eventos.get(self.eventos.curselection()).split(" | ")[0]
            EventoForm(root=self.root, evento=id)
        else:
...
class EventoForm:
    ...
    def __init__(self, root, evento=None, *args, **kwargs):
        ...
        # Guardamos el evento (id) como un atributo en caso de que lo recibamos como argumento.
        self.evento = evento
    ...
    def set_values(self, *args, **kwargs):
        ...
        if self.evento:
            #Si recibimos el id del evento, imprimo todos sus datos en el formulario.
 
            evs = ce.Evento.objects(id=self.evento)
            ...
            # Si no, solo imprimimos las fechas y horas para mostrar formato.
